...
5. Codex workers own planning, divergence, requirements, prototype, UI/UX, full-stack implementation, tests, review, and closure.
...
10. Role boundaries are enforced before Codex can run: planner/divergent/requirements/reviewer/closer are analysis-only, uiux is design-only, prototype is spec-only, fullstack is the only product-code role, and tester may write reports/screenshots/test files but no production source.
11. Frontend UI/UX workers must use project design specs first; if none exist, run `.claude/scripts/sync_design_refs.py --offline --quiet`, consult local `.claude/design-references`, and record `design_reference_selection` plus `style_contract` before implementation. Selected design decisions must be traceable; do not add untraceable beautification. Prefer project/open-license images or video for real visuals and use SVG sparingly. If needed media is missing, UI/UX should record an `asset_generation_brief`; Codex implementation should generate/place bitmap assets locally, record `local_asset_manifest`, and reference local files only.
12. UserPromptSubmit routing and semantic task-input guarding are LLM-backed through `.claude/scripts/llm_router.py`, using `.claude/.env` provider/model settings. Codex CLI `gpt-5.4-mini` with an output schema is preferred for stable JSON decisions. Do not replace semantic routing or role-boundary judgment with regex/keyword rules.
13. Worker tasks record reusable lessons with `taskctl.py experience-add`; closer/reviewer tasks curate them and regenerate the compact learned-experience skill.
14. Use `taskctl.py status` and `taskctl.py audit` for compact state summaries instead of reading full logs by default.
15. `.claude` is not a general write escape hatch. Claude may directly write runtime state such as `.claude/artifacts/**`, `.claude/task-plans/**`, and `.claude/scheduled_tasks.json`; control-plane source/config writes require explicit maintenance mode; product files still go through Codex/taskctl.
16. Add new control-plane behavior as focused modules under `.claude/scripts/` instead of adding unrelated responsibilities to the legacy `taskctl.py` monolith."""
