...
11. Frontend UI/UX workers must use project design specs first; if none exist, run `.claude/scripts/sync_design_refs.py --offline --quiet`, consult local `.claude/design-references`, and record `design_reference_selection` plus `style_contract` before implementation. Selected design decisions must be traceable; do not add untraceable beautification. Prefer project/open-license images or video for real visuals and use SVG sparingly. If needed media is missing, UI/UX should record an `asset_generation_brief`; assetgen should generate/place bitmap assets locally, record `local_asset_manifest`, and reference local files only.
