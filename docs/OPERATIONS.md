...
| Grok stops after PreToolUse denies `run_terminal_command` or `search_replace` | Installed PreToolUse hook is returning a hook error for Grok blocks, or Grok sent a Claude-style `tool_name` payload with a Grok internal alias | Update `.claude/scripts`; current Grok PreToolUse blocks return exit code 0 with `decision=deny` and `continue=true`, and recognize aliases such as `run_terminal_command` and `search_replace` even without `GROK_HOOK_EVENT`. |
| Grok can read workspace JSON with `cd ...; python -c ...` from the controller thread | Installed inline Python scanner only checked commands that start with Python | Update `.claude/scripts`; current hooks scan each shell command segment and block inline Python workspace file operations before the Bash guard runs. |
...
| A Codex worker hangs after starting `npm run dev`, `next dev`, Vite, or `python -m http.server` | A long-running service was started in the foreground, so `taskctl` waits for a command that never exits | Update `.claude/scripts`; current worker prompts require background service lifecycle management. `codex_exec.py` only logs `FOREGROUND_SERVER_WARNING` by default when dev-server readiness text appears; hard termination requires `CODEX_FOREGROUND_SERVER_GUARD_MODE=kill`. |
